...
if (empty($nombre) || empty($apellido) || empty($email) || empty($numero_celular) || empty($password) || empty($fecha_nacimiento)) {
    header("Location: ../../views/admin/user/crear_user.php?error=1");
    exit(); 
}

$stmt = $conx->prepare("INSERT INTO usuarios (nombre, apellido, email, numero_celular, password, fecha_nacimiento) VALUES (?, ?, ?, ?, ?, ?)");
$stmt->bind_param("ssssss", $nombre, $apellido, $email, $numero_celular, $password, $fecha_nacimiento);
$stmt->execute();
$stmt->close();
